    ...
    pub fn update(&mut self, now: Instant, new_stats: StatsMap) -> bool {
        match self {
            ConnState::Connecting {
                ...
                if new_stats.values().all(|stats| stats.rx_bytes > 0) {
            ...
            ConnState::Connected {
                ...
                let mut rx_incremented = !stats.is_empty();
                for (key, peer_stats) in stats.iter() {
                    if let Some(new_peer_stats) = new_stats.get(key) {
                        if new_peer_stats.rx_bytes > peer_stats.rx_bytes {
                            continue;
                        }
                    }
                    rx_incremented = false;
                    break;
                }
